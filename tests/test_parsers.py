...
def test_texto_ocr_crudo_incluido():
    resultado = parsear_campos("  Hola mundo  ")
    assert resultado.texto_ocr_crudo == "Hola mundo"


def test_parsear_comprobante_pichincha_misma_linea():
    texto = """$ 20.00
...
N° de comprobante: 94644154"""
    ...
    assert "N° de comprobante: 94644154" in resultado.texto_ocr_crudo


def test_parsear_no_comprobante_linea_siguiente():
    texto = """BANCO PICHINCHA
Transferencia exitosa!
$8.00
A Galeas Ayala Anita Maria
De Caiza Ocapana Jose Raul.
N° de comprobante
6953"""
    resultado = parsear_campos(texto)
    assert resultado.transfiere == "Caiza Ocapana Jose Raul."
    assert resultado.monto == "8.00"
    assert resultado.no_comprobante == "6953"


def test_parsear_no_comprobante_linea_siguiente_con_corrupcion():
    """N° corrupto a � (U+FFFD) por encoding — comun en OCR.space."""
    texto = """BANCO PICHINCHA
$8.00
A Galeas Ayala Anita Maria
De Caiza Ocapana Jose Raul.
